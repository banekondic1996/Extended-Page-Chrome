// ── Compatibility shim: use browser.* (Firefox) or chrome.* (Chromium) ─────────
/* global browser, chrome */
if (typeof browser === 'undefined') {
  // In Chromium, 'browser' is not defined — alias it to chrome
  // (chrome.* APIs already return Promises in MV3; for MV2 we wrap below)
...
const SETTINGS_KEY  = 'nt_bg_settings';  // separate from the full nt_settings written by newtab.js

// ── helpers (browser.storage.local with chrome.* fallback) ───────────────────
const _bgBsl = (typeof browser !== 'undefined' && browser.storage && browser.storage.local)
  ? browser.storage.local
  : chrome.storage.local;

function csGet(key) {
  const r = _bgBsl.get(key);
  const p = (r && typeof r.then === 'function') ? r : new Promise(res => _bgBsl.get(key, res));
  return p.then(data => (data && data[key] != null) ? data[key] : null).catch(() => null);
}
function csSet(key, value) {
  const r = _bgBsl.set({ [key]: value });
  if (r && typeof r.then === 'function') return r;
  return new Promise(res => _bgBsl.set({ [key]: value }, res));
}
...
async function ensureAlarm() {
  ...
  if (!existing) browser.alarms.create(ALARM_NAME, { periodInMinutes: 30 });
}
...
browser.runtime.onInstalled.addListener(() => { ensureAlarm(); runBackgroundRefresh(); });
browser.runtime.onStartup.addListener(() => { ensureAlarm(); runBackgroundRefresh(); });